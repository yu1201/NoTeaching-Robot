...
$outputDir = Join-Path $repo 'tmp\RobotModePreparationStoreTests'
$outputExe = Join-Path $outputDir 'RobotModePreparationStoreTests.exe'
...
$compile = @(
    ...
    ('"{0}"' -f (Join-Path $repo 'scripts\tests\robot_mode_preparation_store_tests.cpp')),
    ('/Fe:"{0}"' -f $outputExe), '/link', ('/LIBPATH:"{0}"' -f $qtLib),
    'Qt6Core.lib', 'Qt6Network.lib', 'Qt6Sql.lib', 'Crypt32.lib', 'shell32.lib'
) -join ' '
& cmd.exe /d /s /c $compile
if ($LASTEXITCODE -ne 0) { throw "Mode preparation store compilation failed: $LASTEXITCODE" }
$env:PATH = (Join-Path $QtRoot 'bin') + ';' + $env:PATH
$env:QT_PLUGIN_PATH = Join-Path $QtRoot 'plugins'
foreach ($phase in @('seed','restart','verify-revoke')) {
    & $outputExe $suiteRoot $phase
    if ($LASTEXITCODE -ne 0) { throw "Mode preparation store phase ${phase} failed: $LASTEXITCODE" }
}
