...
$outputDir = Join-Path $repo 'tmp\FanucTpContentIdentityTests'
$outputExe = Join-Path $outputDir 'FanucTpContentIdentityTests.exe'
...
$compile = @(
    ...
    ('"{0}"' -f (Join-Path $repo 'src\FanucTpContentIdentity.cpp')),
    ('"{0}"' -f (Join-Path $repo 'scripts\tests\fanuc_tp_content_identity_tests.cpp')),
    ('/Fe:"{0}"' -f $outputExe), ('/Fo{0}\' -f $outputDir),
    '/link', ('/LIBPATH:"{0}"' -f $qtLib), 'Qt6Core.lib'
) -join ' '

& cmd.exe /d /s /c $compile
if ($LASTEXITCODE -ne 0) { throw "FanucTpContentIdentity compilation failed: $LASTEXITCODE" }
$env:PATH = (Join-Path $qtRoot 'bin') + ';' + $env:PATH
& $outputExe
if ($LASTEXITCODE -ne 0) { throw "FanucTpContentIdentity tests failed: $LASTEXITCODE" }
