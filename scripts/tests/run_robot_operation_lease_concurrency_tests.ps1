...
$compile = @(
    ...
    '/DUNICODE', '/D_UNICODE', '/DNOMINMAX', '/DROBOT_OPERATION_LEASE_TEST_STUB_DRIVER',
    ('/I"{0}"' -f (Join-Path $repo 'include')),
    ('/I"{0}"' -f $qtInclude), ('/I"{0}\QtCore"' -f $qtInclude),
    ('/I"{0}\QtNetwork"' -f $qtInclude),
    ('"{0}"' -f (Join-Path $repo 'src\RobotOperationLease.cpp')),
    ('"{0}"' -f (Join-Path $repo 'scripts\tests\robot_operation_lease_concurrency.cpp')),
    ('/Fe:"{0}"' -f $outputExe), ('/Fo{0}\' -f $outputDir),
    '/link', ('/LIBPATH:"{0}"' -f $qtLib), 'Qt6Core.lib', 'Qt6Network.lib'
) -join ' '

& cmd.exe /d /s /c $compile
if ($LASTEXITCODE -ne 0) {
    throw "RobotOperationLeaseConcurrencyTests compilation failed: $LASTEXITCODE"
}
$env:PATH = (Join-Path $qtRoot 'bin') + ';' + $env:PATH
& $outputExe
if ($LASTEXITCODE -ne 0) {
    throw "RobotOperationLeaseConcurrencyTests failed: $LASTEXITCODE"
}
