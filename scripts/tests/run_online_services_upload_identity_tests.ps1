...
$outputDir = Join-Path $repo 'tmp\OnlineServicesUploadIdentityTests'
$outputExe = Join-Path $outputDir 'OnlineServicesUploadIdentityTests.exe'
...
$testSource = Join-Path $repo 'scripts\tests\online_services_upload_identity_tests.cpp'
$compile = @(
...
if ($LASTEXITCODE -ne 0) {
    throw "OnlineServices upload identity test compilation failed: $LASTEXITCODE"
}
$env:PATH = (Join-Path $qtRoot 'bin') + ';' + $env:PATH
& $outputExe
if ($LASTEXITCODE -ne 0) {
    throw "OnlineServices upload identity tests failed: $LASTEXITCODE"
}
