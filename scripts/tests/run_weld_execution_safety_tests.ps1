...
$outputDir = Join-Path $repo 'tmp\WeldExecutionSafetyTests'
$outputExe = Join-Path $outputDir 'WeldExecutionSafetyTests.exe'
...
$compile = @(
    ...
    ('"{0}"' -f (Join-Path $repo 'scripts\tests\weld_execution_safety_tests.cpp')),
    ('/Fe:"{0}"' -f $outputExe), ('/Fo{0}\' -f $outputDir),
    '/link', ('/LIBPATH:"{0}"' -f $qtLib), 'Qt6Core.lib'
) -join ' '

& cmd.exe /d /s /c $compile
if ($LASTEXITCODE -ne 0) { throw "WeldExecutionSafety compilation failed: $LASTEXITCODE" }
$env:PATH = (Join-Path $qtRoot 'bin') + ';' + $env:PATH
& $outputExe
if ($LASTEXITCODE -ne 0) { throw "WeldExecutionSafety tests failed: $LASTEXITCODE" }
