...
def main() -> int:
    ...
    save_settings_start = validity_dialog.index(
        "bool LaserWeldFilterDialog::SaveSettings(")
    save_settings_end = validity_dialog.index(
        "void LaserWeldFilterDialog::LoadExternalAlgorithmConfig()", save_settings_start)
    save_settings = validity_dialog[save_settings_start:save_settings_end]
    require("findChildren<QAbstractSpinBox*>()" in save_settings
            and "spinBox->interpretText();" in save_settings
            and save_settings.index("spinBox->interpretText();")
                < save_settings.index("m_pValidationMinLongestContinuousSpin->value()"),
            "save can read stale spin-box values before pending editor text is committed")
