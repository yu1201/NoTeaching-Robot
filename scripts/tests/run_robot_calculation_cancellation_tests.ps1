...
$outputDir = Join-Path $repo 'tmp\RobotCalculationCancellationTests'
$outputExe = Join-Path $outputDir 'RobotCalculationCancellationTests.exe'
...
    ('"{0}"' -f (Join-Path $repo 'src\Const.cpp')),
    ...
    ('"{0}"' -f (Join-Path $repo 'scripts\tests\robot_calculation_cancellation_tests.cpp')),
...
if ($LASTEXITCODE -ne 0) {
    throw "RobotCalculation cancellation test compilation failed: $LASTEXITCODE"
}
$env:PATH = (Join-Path $qtRoot 'bin') + ';' + $env:PATH
& $outputExe
if ($LASTEXITCODE -ne 0) {
    throw "RobotCalculation cancellation tests failed: $LASTEXITCODE"
}
