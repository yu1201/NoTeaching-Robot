# -*- coding: utf-8 -*-
"""
明图激光传感器 FANUC 宏 —— 数学参考实现 (ground-truth oracle)
...
def MT_YX(p1, p2, p3, p4, posture, method):
...
def run_tests():
    ...
    # MT_CZ: 点(2,3,0) 投到 X轴(0,0,0)-(10,0,0) -> 垂足(2,0,0)
    r, f = MT_CZ(P(0,0,0), P(10,0,0), P(2,3,0), posture); check("MT_CZ 垂足", r, (2,0,0))
    # 姿态应拷贝自 posture
    print("        MT_CZ 姿态拷贝:", tuple(round(v,3) for v in r[3:]), "应=", (30.0,0.0,90.0))

    # MT_YC: p1(0,0,0)->p2(0,10,0) 再外延 5 -> (0,15,0)
    r, f = MT_YC(P(0,0,0), P(0,10,0), posture, 5); check("MT_YC 延长", r, (0,15,0))

    # MT_JD: 直线 X轴 (y=0) 与 直线 (x=4 的竖线) 交于 (4,0,0)
    r, f = MT_JD(P(-5,0,0), P(5,0,0), P(4,-3,0), P(4,7,0), posture); check("MT_JD 交点", r, (4,0,0))

    # MT_YX 三点法: 单位圆上三点 -> 圆心(0,0,0)
    r, f = MT_YX(P(1,0,0), P(0,1,0), P(-1,0,0), P(0,0,0), posture, 1); check("MT_YX 三点圆心", r, (0,0,0))
    # MT_YX 四点法: 圆心(2,3,0) 半径5 上四点
    c=(2,3,0); R5=5
    pp=[P(c[0]+R5,c[1],0),P(c[0],c[1]+R5,0),P(c[0]-R5,c[1],0),P(c[0],c[1]-R5,0)]
    r, f = MT_YX(pp[0],pp[1],pp[2],pp[3], posture, 2); check("MT_YX 四点圆心", r, c, 1e-5)

    # MT_YH 三点法: 原点(0,0,0), P2 在 +X, P3 在 +Y -> 单位阵姿态(0,0,0)
    r, f = MT_YH(P(0,0,0), P(10,0,0), P(0,10,0), P(0,0,0), 1)
    check("MT_YH 原点", r, (0,0,0)); print("        MT_YH 姿态wpr:", tuple(round(v,3) for v in r[3:]), "应≈(0,0,0)")
    # P2 在 +X, P3 在 +Y => ex=(1,0,0) ey=(0,1,0) ez=(0,0,1) => wpr=(0,0,0)

    # MT_YH 绕Z转90°: X轴指向+Y -> r=90
    r, f = MT_YH(P(0,0,0), P(0,10,0), P(-10,0,0), P(0,0,0), 1)
    print("        MT_YH 绕Z90 wpr:", tuple(round(v,3) for v in r[3:]), "应≈(0,0,90)")
