...
private:
    ...
    void PopulateSourceDefaults()
    {
        ...
        const T_ROBOT_COORS tool = m_driver != nullptr ? m_driver->m_tTools.tGunTool : T_ROBOT_COORS();
    ...
    void ImportRuntimeDh()
    {
        ...
        const std::array<double, kDhParamCount> parameters = KinematicsToParamArray(m_driver->m_tKinematics);
        for (int joint = 0; joint < kDhJointCount; ++joint)
        {
            for (int field = 0; field < 4; ++field)
            {
