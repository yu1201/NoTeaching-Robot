...
管理调试中的扫描变姿态测试可在“直线处理”扫描成功后执行一轮焊枪空跑。输入只接受当前机器人 `Result/<robot>/<case>/LaserPoint/PreciseLaserPoint_FeatureSmoothCurve_2mm.txt` 及其匹配的直线处理摘要、扫描成功摘要；曲线 XYZ 是 Tool1 TCP，RX/RY/RZ 使用已示教基础姿态，按文件原顺序和固定 2 mm 点距生成程序。流程为：计算首点下枪安全位 → 到曲线起点 → 适配层下发并 START → 等待控制器正常完成 → 末点收枪安全位并回读验证。该来源在生成和执行两侧都强制 `actualWeld=false`，不能反向、续焊或改点距，也不使用焊接工艺摆动/补偿。
