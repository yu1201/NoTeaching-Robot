...
仅在服务器缺少固定账号时，为新建的 `devicedata`、`uploader` 分别准备一行密码文件。文件应由 root 持有，权限为 0600；
...
已有账号必须省略对应密码文件并保留原密码；为已有账号传密码文件会直接失败，密码轮换只能通过
`ota-admin` 在安全管理通道中完成。新服务器缺少账号且未提供密码文件时部署也会失败。
...
- 两者属于 `ftpdata`，共享目录 `/srv/devicedata/data` 为 2770/setgid，`local_umask=002`。
...
UFW、`chpasswd` 或生产网络。正式上线仍需在目标 Ubuntu 上做 `devicedata/uploader` 登录、
上传、下载拒绝、删除拒绝、NAT 被动连接和重启后服务自启验收。
