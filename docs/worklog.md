...
- 版本与发布
  - 应用版本更新为 `v2026.06.11.1021`（自 v2026.06.10.2247 起依次并入 SDK 20260609 更新、截断方向修复、预览方向箭头、焊接方向迁工艺、扫描超时动态化、箭头侧偏按扫描位置取边，逐次重新发版替代）
  - `Debug x64` 与 `Release x64` 编译通过；Inno Setup 打包通过，生成 `NoTeaching-Robot-Setup-v2026.06.11.1021.exe`
  - 安装包大小 `101,231,063` bytes，SHA256 `035A55317CA96E56317948F911B74E6548085E1D5C0E2F902C8AFF427E98552A`
  - 已验证包内 `BUILD_VERSION.txt` 为 `2026.06.11.1021`、包内 SDK dll 与厂商更新包字节一致；安装包文件名/注册表版本/应用版本三者一致（打包脚本默认取打包时刻为版本，发版需显式传 -AppVersion 与源码同步）
