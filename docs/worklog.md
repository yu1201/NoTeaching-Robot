...
- 版本与发布
  - 应用版本更新为 `v2026.06.11.0956`（自 v2026.06.10.2247 起依次并入 SDK 20260609 更新、截断方向修复、预览方向箭头、焊接方向迁工艺、扫描超时动态化，逐次重新发版替代）
  - `Debug x64` 与 `Release x64` 编译通过；Inno Setup 打包通过，生成 `NoTeaching-Robot-Setup-v2026.06.11.0956.exe`
  - 安装包大小 `101,234,239` bytes，SHA256 `EC7231B85A4E94E975C6F4DD9E0A3BAA93D18E737C2F7ECB88839F16BA998D34`
  - 已验证包内 `BUILD_VERSION.txt` 为 `2026.06.11.0956`、包内 SDK dll 与厂商更新包字节一致；安装包文件名/注册表版本/应用版本三者一致（打包脚本默认取打包时刻为版本，发版需显式传 -AppVersion 与源码同步）
