...
- 工件点云逆向建模与三维查看
  ...
  - 新增工件模型查看器（QOpenGLWidget 硬件渲染）：实体面（光照+高度伪彩）/点云/2.5D 高度图三种模式，补偿界面预览工具栏"工件模型"按钮打开；老目录首次打开自动补生成缓存
...
- 版本与发布
  - 应用版本更新为 `v2026.06.11.1333`（自 v2026.06.10.2247 起依次并入 SDK 20260609 更新、截断方向修复、预览方向箭头、焊接方向迁工艺、扫描超时动态化、箭头侧偏按扫描位置取边、LOGPATH 空值兜底、四方法独立基础焊道与缺失自动重算、工件模型缓存与三维查看器，逐次重新发版替代）
  - `Debug x64` 与 `Release x64` 编译通过；Inno Setup 打包通过，生成 `NoTeaching-Robot-Setup-v2026.06.11.1333.exe`
  - 安装包大小 `101,580,235` bytes，SHA256 `D1244859ECFD779BA3F6DCCB38EB42BB4CD48DA2AC6D42C3C862B5ECA3C2B314`
  - 已验证包内 `BUILD_VERSION.txt` 为 `2026.06.11.1333`、Qt6OpenGL/Qt6OpenGLWidgets 随包部署、SDK dll 与厂商更新包字节一致；安装包文件名/注册表版本/应用版本三者一致（打包脚本默认取打包时刻为版本，发版需显式传 -AppVersion 与源码同步）
