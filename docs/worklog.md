...
- 本地发布候选 v2026.09.08.2143：完整同步“底层优化适配”远端提交及本地有效源码快照，共覆盖汇川连接/登录/模式/运动学/标定/验收、STEP/FANUC 统一适配、扫描变姿态与 Tool1 曲线空跑、系统联锁、配置原子保存、机器人操作租约及中断安全恢复；新版点云 SDK 直返焊道、进程隔离和 SDKBase 完整性门禁继续保留。运行生成的 Job、output、缓存及本机配置未提交。专项 C++ 与静态回归通过，双分支 Release 与安装包门禁进行中。署名 yu1201。
