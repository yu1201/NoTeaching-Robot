...
- 发布 v2026.09.09.1850：同步在线服务、现场数据上传、FTP 账号/设备名称、新版点云 SDK 调用及授权机器标识更新；修复点云参数保存回退、安全门禁互相覆盖和首尾截断平台伪拐点。正式发布工具改为绑定现有 main 与 hk-pathlynx-corpla 固定目录，各自使用本分支源码、SDK、图标和资源构建，只校验 FANUC 运行文件，不再创建临时 worktree 或跨分支复制文件；中性版保持 neutral/Off，品牌版保持 brand/Enforce 与生产公钥。GitHub、OTA 和双通道安装包按同一版本发布，不上传 GitLab。署名 yu1201。
