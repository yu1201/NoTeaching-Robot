...
- 补偿界面
  ...
  - "原始数据"图层按当前方法显示真实处理输入：①SDK全处理显示局部完整点云（流式抽样上限 6 万点）、②SDK+拟合显示 SDK 基础焊道、③④显示相机目标点轨迹；对应文件缺失时回退轨迹点，日志标注数据源类型与点数
...
- 版本与发布
  - 应用版本更新为 `v2026.06.11.1102`（自 v2026.06.10.2247 起依次并入 SDK 20260609 更新、截断方向修复、预览方向箭头、焊接方向迁工艺、扫描超时动态化、箭头侧偏按扫描位置取边、LOGPATH 空值兜底、原始数据按方法取源，逐次重新发版替代）
  - `Debug x64` 与 `Release x64` 编译通过；Inno Setup 打包通过，生成 `NoTeaching-Robot-Setup-v2026.06.11.1102.exe`
  - 安装包大小 `101,228,394` bytes，SHA256 `9F844D888C788BCA7957645DBD88CE42BCC75474BA5B465C87478C852047DF24`
  - 已验证包内 `BUILD_VERSION.txt` 为 `2026.06.11.1102`、包内 SDK dll 与厂商更新包字节一致；安装包文件名/注册表版本/应用版本三者一致（打包脚本默认取打包时刻为版本，发版需显式传 -AppVersion 与源码同步）
