...
- 版本与发布
  - 应用版本更新为 `v2026.06.10.2247`
  - `Debug x64` 与 `Release x64` 编译通过；Inno Setup 打包通过，生成 `NoTeaching-Robot-Setup-v2026.06.10.2247.exe`
  - 安装包大小 `101,276,321` bytes，SHA256 `F151CB1C3F88DA7C6675B938227AD572637F9C9B57993904E187866F7BDB58C8`
  - 已验证包内 `BUILD_VERSION.txt` 为 `2026.06.10.2247`，安装包文件名/注册表版本/应用版本三者一致（打包脚本默认取打包时刻为版本，本次显式传 -AppVersion 与源码同步）
