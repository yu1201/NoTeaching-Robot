...
- 发布 v2026.09.07.2052：更新点云焊道提取 SDK、导入库及 OpenCV/PCL/MSVC 运行时闭包；调用侧兼容新版库直接返回稠密基础焊道且不再生成 `Save_File_Name` 文件的行为，通过点数、总长度、中位点距和稠密段比例门禁后继续原有拟合与质量授权链。同步纳入主分支已测试的 SDKBase 完整性门禁，在平滑、截断、拟合和平台重算前校验完整点云覆盖率与端点偏差，并将阈值、实测值和拒绝原因写入质量证明及点云界面。SDK 路径增加无损编码检查，隔离子进程超时按点云规模扩展至 300–900 秒；异常输出、崩溃、超时和依赖缺失继续失败关闭。`底层优化适配` 中汇川、STEP、扫描变姿态等测试功能不进入本次发布。双通道 Release、安装包、OTA、GitHub、GitLab及版本台账按正式发布门禁执行。署名 yu1201。

## 2026-08-24

- 发布 v2026.08.24.1333：汇总机器人适配层与关节/脉冲缺省值、模型焊接候选与逆向投影、扫描位姿变化测试、跨 Windows 用户配置修复、FTP 程序数量提醒及源码离线环境配套。机器人/焊接静态安全门禁、84 项配置库回归、三组几何/流程 C++ 测试、双通道 Release 与安装包配对门禁均已通过，并经用户确认进入正式发布；发布流程同步刷新已过期的 OTA v2/v3 签名清单，保持 7 天新鲜度和回滚防护。署名 yu1201。
