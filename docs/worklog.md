...
- 发布 v2026.09.09.1850：同步在线服务、现场数据上传、FTP 账号/设备名称、新版点云 SDK 调用、FANUC 服务运行文件及授权机器标识更新；修复点云参数保存回退、安全门禁互相覆盖和首尾截断平台伪拐点。正式发布工具改为绑定现有 main 与 hk-pathlynx-corpla 固定目录，各自使用本分支源码、SDK、图标和资源构建，只校验 FANUC 运行文件，不再创建临时 worktree 或跨分支复制文件；中性版保持 neutral/Off，品牌版保持 brand/Enforce 与生产公钥。GitHub、OTA 和双通道安装包按同一版本发布，不上传 GitLab。署名 yu1201。

- 发布 v2026.09.09.0132：在昨晚全分支完整同步基础上接入独立软件授权客户端，中性版默认关闭、品牌版使用生产授权公钥强制验签；授权异常统一阻止新机器人任务，并保留可验证安全停止与安全回撤。坡口相机预览改为先显示界面再异步连接，离线状态在界面内提示；控制单元管理恢复显示已配置但未启用的机器人。授权服务端 31 项、Windows 客户端 54 项、机器人授权门禁并发测试及相机/安全静态回归均通过；运行数据、日志、私钥、管理员令牌和授权数据库不提交。署名 yu1201。

## 2026-09-08

- 发布 v2026.09.08.2143：完整同步“底层优化适配”远端提交及本地有效源码快照，并保留 main 已验证的安装、迁移、发布和恢复安全更新；覆盖汇川连接/登录/模式/运动学/标定/验收、STEP/FANUC 统一适配、扫描变姿态与 Tool1 曲线空跑、系统联锁、配置原子保存、机器人操作租约及中断安全恢复。新版点云 SDK 直返焊道、进程隔离和 SDKBase 完整性门禁继续保留；机器人厂家源码默认账号及 FTP 密码按固定适配参数保留。运行生成的 Job、output、缓存及本机配置未提交。全量专项 C++、静态回归、Release x64、安装包恢复、配置迁移一致性和双通道打包门禁均通过，按同一 main 提交同步中性版与品牌版并发布 GitHub Release，不上传 GitLab。署名 yu1201。

## 2026-08-28

- 开发中：扫描变姿态精度测试的“上坡左旋角度”和“下坡右旋角度”由仅允许 `0~60 deg` 改为允许 `-60~60 deg`，负值表示反转对应字段的命名旋转方向；服务层同步按绝对值校验范围及非零过渡条件，轨迹摘要同时显示输入值与实际 RotZ，既保留原有正值行为，又避免负数摘要出现双重符号歧义。Debug/Release x64 与专项静态回归通过。未提交、未发布。署名 yu1201。

## 2026-08-27

- 开发中：扫描变姿态精度测试在“直线处理”成功后增加“生成并运行直线模拟轨迹”。入口读取当前机器人案例内严格绑定的 `PreciseLaserPoint_FeatureSmoothCurve_2mm.txt`，将曲线 XYZ 直接作为 Tool1 焊枪 TCP，全部点统一使用已示教基础姿态，按原顺序生成固定 2 mm 空跑程序；输入需同时通过曲线表头/点距/来源标签、直线处理摘要及扫描成功摘要校验。本流程强制不开弧、不送丝、不摆动、不应用焊道或姿态补偿，并复用先测后焊的下枪安全位、起点、适配层下发/START、程序完成回读、收枪安全位和持久安全恢复门禁；虚拟焊道与扫描曲线的进程内授权类型互不复用。未提交、未发布。署名 yu1201。
