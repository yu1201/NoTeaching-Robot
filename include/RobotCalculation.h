...
public:
    ...
    struct LowerWeldFilterParams
    {
        ...
        // 再用典型拐角弯折角确认，补回起点/终点段被检测盲区漏掉的拐点。默认关。
        ...
        // 同类短段多余拐点合并：与端区补点独立。排除首末/搭接段，将中间完整段按 IO/OI/II/OO
        // 四类分别统计；候选必须短、实际为坡且删后恢复到同方向坡典型长度，才自动合并。
        bool enableSameTypeShortCornerMerge = false;
        double endPeriodMergeFrac = 0.4;       // 候选短段 < 此×同类完整段中位长度
        int sameTypeShortMinReferenceSegments = 2;
        double sameTypeShortFlatSlope = 0.15;
